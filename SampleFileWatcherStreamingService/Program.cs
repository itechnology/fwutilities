...
namespace FileWatcherUtilities.SampleFileWatcherStreamingService
{
    ...
    class Program
    {
        static void Main()
        {
            Uri baseAddress = new Uri("http://localhost:8000/SampleFileWatcherStreamingService");

            using (ServiceHost serviceHost = new ServiceHost(typeof(FileWatcherStreamingService), baseAddress))
            {
                try
                {
                    BasicHttpBinding basicHttpBinding = new BasicHttpBinding();
                    basicHttpBinding.TransferMode = TransferMode.StreamedRequest;
                    basicHttpBinding.MaxReceivedMessageSize = 20480000;

                    serviceHost.AddServiceEndpoint(typeof(IFileWatcherStreamingService),
                                                   basicHttpBinding,
                                                   "SampleFileWatcherStreamingService");
